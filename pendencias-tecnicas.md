...
- **Tela administrativa de regras.** A API já existe (`GET/PATCH
  /compliance-engine/regras`, `GET /compliance-engine/tipos-documento`,
  admin-only), falta a interface em Configurações/Compliance pra editar
  prazo/bloqueante/base legal sem precisar de acesso direto ao banco.
- **Aprovações** — quando uma pendência exige assinatura, nascer como item de
  Aprovações (fluxo de assinatura eletrônica via provedor ICP-Brasil).
- **Elô (OCR)** — ao anexar o documento que resolve uma pendência, validar se
  o arquivo bate com o `tipo_documento` esperado.
- **Portal do Colaborador** — exibir lá as pendências que dependem de
  assinatura do próprio colaborador.
- **Indicadores** — card/gráfico do Índice de Conformidade (geral, por setor,
  por colaborador).
