  ...
  return (
    <div className="flex flex-col gap-8">
            ...
            {block.ferramentas.map((tool) =>
              tool.href ? (
                <Link
                  key={tool.nome}
                  href={tool.href}
                  className="flex flex-col gap-2 rounded-container border border-border bg-surface p-4 transition hover:border-accent"
                >
                  <ToolIcon nome={tool.nome} className="h-5 w-5 text-accent" />
                  <span className="text-sm font-medium text-text">{tool.nome}</span>
                </Link>
              ) : (
                <div key={tool.nome} className="flex flex-col gap-2 rounded-container border border-border bg-surface-alt p-4 opacity-60">
      ...
      <section className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          {atalhos?.map((a) => (
            <div key={a.id} className="group relative flex flex-col gap-2 rounded-container border border-border bg-surface p-4 transition hover:border-accent">
