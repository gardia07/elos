...
export class AdmissionsService {
  ...
  private async checklistComplete(
    filial: string,
  ...
  async toggleDoc(id: string, dto: ToggleDocDto) {
    ...
    const docs = {
      ...(admission.docs as Record<string, boolean>),
      [dto.key]: dto.checked,
    };
  ...
  async sendEsocial(id: string) {
    const admission = await this.mustFind(id);
    const complete = await this.checklistComplete(
      admission.filial,
      admission.docs as Record<string, boolean>,
    );
    if (!complete)
      throw new BadRequestException('Checklist de documentos incompleto.');
  ...
  async signContract(id: string) {
    ...
    if (!admission.contratoGerado)
      throw new BadRequestException('Gere o contrato antes de assiná-lo.');
    if (admission.contratoAssinado)
  ...
  async efetivar(id: string) {
    const admission = await this.mustFind(id);
    if (admission.status === 'EFETIVADO')
      throw new BadRequestException('Admissão já efetivada.');
    ...
    if (!complete)
      throw new BadRequestException('Checklist de documentos incompleto.');
    if (!admission.esocialSent)
      throw new BadRequestException('Evento eSocial S-2200 não enviado.');
    if (!admission.contratoAssinado)
      throw new BadRequestException('Contrato ainda não assinado.');
    ...
    const employee = await db.employee.create({
      data: {
        ...
        cargoSalarioHistorico: {
          create: [
            {
              vigenciaDesde: admission.dataInicio,
              cargo: admission.cargo,
              salario: admission.salario,
              motivo: 'Admissão',
